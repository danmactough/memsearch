...
for skill_name in memory-recall memsearch-summarize; do
...
# --- 3. Replace __INSTALL_DIR__ placeholders in skills ---
echo "[3/6] Configuring skill paths..."
for skill_file in "$HOME/.agents/skills/memory-recall/SKILL.md" "$HOME/.agents/skills/memsearch-summarize/SKILL.md"; do
  if [ -f "$skill_file" ]; then
    replace_text_in_file "$skill_file" "__INSTALL_DIR__" "$INSTALL_DIR"
  fi
done
echo "  ✓ Updated skills with install path: $INSTALL_DIR"
...
echo "  • UserPromptSubmit: reminds Codex about memory-recall and summarize skills"
echo "  • memory-recall skill: search past memories when relevant"
echo "  • memsearch-summarize skill: /memsearch-summarize saves current or all sessions"
echo ""
echo "Memory files:   <project>/.memsearch/memory/*.md"
echo "Hooks config:   $HOOKS_FILE"
echo "Skill location: $HOME/.agents/skills/{memory-recall,memsearch-summarize}"
